...
public class UIManager : MonoBehaviour
{
    ...
    public GameObject content_lista_usuarios;
    public Item_lista_usuario item_lista_usuario;
    ...
    /// <summary>
    /// Método para crear la lista de usuarios en el content del scrow view de la UI
    /// Creará un elemento por cada usuario existente en la lista
    /// </summary>
    public void Cargar_Info_Lista_Usuarios()
    {
        Limpiar_Lista_Usuarios();
        
        // Recorremos la lista de usuarios y por cada usuario creamos dentro del content un elemento de tipo
        // Item_lista_usuario con la info de el usuario
        foreach (var usuario in datos_Usuarios.listaUsuarios)
        {
            Item_lista_usuario _item_lista_usuario;
            _item_lista_usuario = Instantiate(item_lista_usuario, content_lista_usuarios.transform);
            _item_lista_usuario.nombre = usuario.nombre;
            _item_lista_usuario.email = usuario.email;
            _item_lista_usuario.name = usuario.email;
        }
        
    }

    public Action Actualizar_Info_Persona_Seleccionada(string nombrePersona, string emailPersona)
    {
        nombre_usuario_panel_listaUsuarios.text = nombrePersona;
        email_usuario_panel_listaUsuarios.text = emailPersona;

        return null;
    }

    /// <summary>
    /// Método para destruir todos los hijos del content de la UI lista usuarios
    /// </summary>
    public void Limpiar_Lista_Usuarios()
    {
        foreach (Transform child in content_lista_usuarios.transform)
        {
    ...
    public void CrearUsuario()
    {
            ...
            && !string.IsNullOrEmpty(password_usuario_crear_inputField.text))
        {
            ...
            // Vacio mi lista temporal de usuarios
            ...
            if (existeUsuarioConMismosDatos == false)
            {
                ...
                // Llamar al web request para subir la info a la nube
